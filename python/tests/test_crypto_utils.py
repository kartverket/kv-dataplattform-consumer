...
def test_that_encrypted_then_decrypted_data_returns_original_asymmetric():
    ...
    # Act
    aes_key_enc = asymmetric_encrypt_symmetric_key(
        aes_key_data, rsa_key_pair["public_key"]
    )
    sample_data_dec = asymmetric_decrypt_symmetric_key(
        aes_key_enc, rsa_key_pair["private_key"]
    )

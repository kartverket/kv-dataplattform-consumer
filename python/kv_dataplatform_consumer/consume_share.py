...
from kv_dataplatform_consumer.crypto_utils import (
    symmetric_decrypt_data,
    asymmetric_decrypt_symmetric_key,
)
...
def consume_pii_table(
    df_table: pd.DataFrame, df_key_table: pd.DataFrame, asymmetric_private_key: bytes
) -> pd.DataFrame:
    ...
    for enc_col in encrypted_columns:
        ...
        if nonce_col in joined_df.columns:
            joined_df[col_name] = joined_df.apply(
                lambda row: symmetric_decrypt_data(
                    row[enc_col], key_dec, row[nonce_col]
                ),
                axis=1,
            )

            logging.info(
                f"Decrypted column '{col_name}': {joined_df[col_name].tolist()}"
            )
...
def consume_table_from_share(
    ...
    df_table_keys["key"] = df_table_keys["key"].apply(
        lambda key_enc: base64.standard_b64decode(key_enc)
    )
